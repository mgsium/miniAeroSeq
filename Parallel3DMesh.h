 ...
 * For perfect load-balancing number of points in each direction must be a power
 * of 2. Ghosting information is also determined by this class. This class could
 * potentially be replaced by a read-in from an unstructured mesh database file.
 */

class Parallel3DMesh {
 public:
  Parallel3DMesh(int nx, int ny, int nz, double lx, double ly, double lz,
                 int problem_type, double angle = 0.0);
  ~Parallel3DMesh();
  ...
  void getGhostOffset(std::vector<int>& ghost_offset) {
    ...
    if (nprocx_ != 1 && x_block_ != 0) ghost_offset[0] = 1;
    if (nprocy_ != 1 && y_block_ != 0) ghost_offset[1] = 1;
    if (nprocz_ != 1 && z_block_ != 0) ghost_offset[2] = 1;
  }
  ...
  inline int getNumberGhostedElements() {
    ...
    return ghost_dir[0] * ny_ * nz_ + ghost_dir[1] * nx_ * nz_ +
           ghost_dir[2] * nx_ * ny_;
  }
  ...
  inline int getNumberNodes() {
    ...
    return (nx_ + ghost_dir[0] + 1) * (ny_ + ghost_dir[1] + 1) *
           (nz_ + ghost_dir[2] + 1);
  }
  ...
  inline int getNumberGhostedNodes() {
    ...
    return ghost_dir[0] * (ny_ + 1) * (nz_ + 1) +
           ghost_dir[1] * (nx_ + 1) * (nz_ + 1) +
           ghost_dir[2] * (nx_ + 1) * (ny_ + 1);
  }

  void fillMeshData(struct MeshData& mesh_data) {
    ...
    time(&faceStartTime);
    create_faces(element_node_conn, mesh_faces, node_coordinates, num_elems,
                 num_nodes);
    time(&faceEndTime);
    ...
    compute_cell_volumes(mesh_cells, element_node_conn, node_coordinates,
                         num_elems, num_ghosted_elems);
    myfile << "Done computing cell volumes" << std::endl;
    compute_cell_centroid(mesh_cells, element_node_conn, node_coordinates,
                          num_elems);
...
// Parallel communication setup
#if WITH_MPI
    if (my_id_ == 0) std::cout << "Start setup communication." << std::endl;
    std::vector<std::pair<int, int> > sendProcIdent;
    std::vector<std::pair<int, int> > recvProcIdent;

    time_t commStartTime = 0, commEndTime = 0;
    time(&commStartTime);
    setupCommunication(element_global_id, num_ghosted_elems, sendProcIdent,
                       recvProcIdent, mesh_data.sendCount, mesh_data.recvCount);
    time(&commEndTime);
    ...
    for (int i = 1; i < num_procs_; ++i) {
      sendProcessorOffset[i] +=
          sendProcessorOffset[i - 1] + mesh_data.sendCount[i - 1];
      recvProcessorOffset[i] +=
          recvProcessorOffset[i - 1] + mesh_data.recvCount[i - 1];
    }
    ...
    if (my_id_ == 0) std::cout << "End setup communication." << std::endl;
#endif
    ...
    fprintf(stdout,
            "\n CPU Memory Usage (Current, High Water) - after mesh setup: %lu "
            "MB, %lu MB",
    ...
    Faces internal_faces(ninternal_faces, 2);
    ...
    Faces top_boundary_faces(ntop_faces, 1);
    Faces bottom_boundary_faces(nbottom_faces, 1);
    Faces right_boundary_faces(nright_faces, 1);
    Faces left_boundary_faces(nleft_faces, 1);
    ...
    Cells device_cells(
        num_elems,
        faces_per_element);  // ncells, faces/element. - Need centroid?
    ...
    for (int i = 0; i < ncells; i++) {
      ...
      for (int j = 0; j < 3; j++) {
        device_cells.coordinates_[j][i] = tmp_coord[j];
      }
    }
    ...
    if (problem_type_ == 1)
      mesh_data.boundary_faces.push_back(
          std::pair<std::string, Faces>("NoSlip", bottom_boundary_faces));
    else
      mesh_data.boundary_faces.push_back(
          std::pair<std::string, Faces>("Tangent", bottom_boundary_faces));
    if (problem_type_ == 1)
      mesh_data.boundary_faces.push_back(
          std::pair<std::string, Faces>("Extrapolate", top_boundary_faces));
    else
      mesh_data.boundary_faces.push_back(
          std::pair<std::string, Faces>("Tangent", top_boundary_faces));

    mesh_data.boundary_faces.push_back(
        std::pair<std::string, Faces>("Tangent", front_boundary_faces));
    mesh_data.boundary_faces.push_back(
        std::pair<std::string, Faces>("Tangent", back_boundary_faces));
    mesh_data.boundary_faces.push_back(
        std::pair<std::string, Faces>("Extrapolate", right_boundary_faces));

    if (problem_type_ == 0)
      mesh_data.boundary_faces.push_back(
          std::pair<std::string, Faces>("Extrapolate", left_boundary_faces));
    else
      mesh_data.boundary_faces.push_back(
          std::pair<std::string, Faces>("Inflow", left_boundary_faces));
    ...
    /*#ifdef WITH_MPI
      ...
      id_map_type_host send_local_ids_host =
    Kokkos::create_mirror(send_local_ids); id_map_type_host recv_local_ids_host
    = Kokkos::create_mirror(recv_local_ids);
    ...
    shared_volumes("SharedVolumes",total_send_count); typename
    Kokkos::View<double*,Device>::HostMirror host_shared_volumes =
    Kokkos::create_mirror_view(shared_volumes);
    ...
    typename Kokkos::View<double*,Device>::HostMirror host_ghost_volumes =
    Kokkos::create_mirror_view(ghost_volumes);

    communicate_ghosted_cell_data(mesh_data.sendCount, mesh_data.recvCount,
    host_shared_volumes.ptr_on_device(),host_ghost_volumes.ptr_on_device(), 1);
 ...
 private:
  int getNodeLocalIndex(int i, int j, int k) {
    ...
    return (i + ghost_offset[0]) *
               ((ny_ + 1 + ghost_dir[1]) * (nz_ + 1 + ghost_dir[2])) +
           (j + ghost_offset[1]) * (nz_ + 1 + ghost_dir[2]) +
           (k + ghost_offset[2]);
  }

  inline int getElementGlobalId(int i, int j, int k) {
    return (nx_offset_ + i) * (global_ny_ * global_nz_) +
           (ny_offset_ + j) * global_nz_ + (nz_offset_ + k);
  }
  ...
  std::vector<double> getNodeCoordinate(int i, int j, int k) {
    ...
    if (coordinate[0] < lx_ / 2.0) {
      ...
      double y_ramp =
          (coordinate[0] - (lx_ / 2.0)) * tan(ramp_angle * PI / 180.0);
  ...
  void getElementNodeConnectivities(std::vector<int>& element_node_conn,
                                    std::vector<int>& elem_global_ids);
  void getGhostedElementNodeConnectivities(std::vector<int>& element_node_conn,
                                           std::vector<int>& elem_global_ids);
  ...
  void setupCommunication(std::vector<int>& elem_global_ids,
                          int num_ghosted_elements,
                          std::vector<std::pair<int, int> >& sendProcIdent,
                          std::vector<std::pair<int, int> >& recvProcIdent,
