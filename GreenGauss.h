...
/*green_gauss_face
 * functor to compute internal face contributions for Green-Gauss gradient
 * computation.
 */
struct green_gauss_face {
  ...
  green_gauss_face(Faces faces, Cells cells)
  ...
  void operator()(const int &ii, Cells cells,
                  solution_field_type cell_values_) const {
    ...
    const double left_r = cell_values_[0][left_index];
    const double left_ri = 1.0 / left_r;
    const double left_u = cell_values_[1][left_index] * left_ri;
    const double left_v = cell_values_[2][left_index] * left_ri;
    const double left_w = cell_values_[3][left_index] * left_ri;
    const double left_k =
        0.5 * (left_u * left_u + left_v * left_v + left_w * left_w);
    const double left_e = cell_values_[4][left_index] * left_ri - left_k;
    const double left_T = left_e * (gamma - 1.0) / Rgas;

    const double primitives_l[5] = {left_r, left_u, left_v, left_w, left_T};

    const double right_r = cell_values_[0][right_index];
    const double right_ri = 1.0 / right_r;
    const double right_u = cell_values_[1][right_index] * right_ri;
    const double right_v = cell_values_[2][right_index] * right_ri;
    const double right_w = cell_values_[3][right_index] * right_ri;
    const double right_k =
        0.5 * (right_u * right_u + right_v * right_v + right_w * right_w);
    const double right_e = cell_values_[4][right_index] * right_ri - right_k;
    const double right_T = right_e * (gamma - 1.0) / Rgas;

    const double primitives_r[5] = {right_r, right_u, right_v, right_w,
                                    right_T};
    ...
    for (int idir = 0; idir < 3; idir++) {
      const double face_norm = face_normal_[idir][i];

      for (int icomp = 0; icomp < 5; ++icomp) {
        const double gradient =
            0.5 * (primitives_l[icomp] + primitives_r[icomp]) * face_norm;

#ifdef ATOMICS_FLUX
        /*             double * left_cell =
         &cell_gradient_(left_index,0,icomp,idir);
   Kokkos::atomic_add(left_cell, gradient/cell_vol_left);

                     double * right_cell =
         &cell_gradient_(right_index,0,icomp,idir);
                     Kokkos::atomic_add(right_cell,
   -gradient/cell_vol_right);*/

        cell_gradient_[icomp][idir][left_index][0] += gradient / cell_vol_left;
        cell_gradient_[icomp][idir][right_index][0] -= gradient / cell_vol_left;
#endif

        /*#ifdef CELL_FLUX
                    cell_gradient_[left_index][cell_ind_0][icomp][idir] =
        gradient/cell_vol_left;
                    cell_gradient_[right_index][cell_ind_1][icomp][idir] =
        -gradient/cell_vol_right; #endif*/
      }
    }
  }
};
 ...
 * functor to compute boundary face contributions for Green-Gauss gradient
 * computation.
 */
struct green_gauss_boundary_face {
  ...
  green_gauss_boundary_face(Faces faces, Cells cells)
      : cell_volumes_(cells.volumes_)
  // cell_gradient_(cells.cell_gradient_)
  {
    std::copy(faces.face_cell_conn_, faces.face_cell_conn_ + 2,
  ...
  void operator()(int i, Cells cells, solution_field_type cell_values) const {
    ...
    const double r = cell_values_[0][index];
    const double ri = 1.0 / r;
    const double u = cell_values_[1][index] * ri;
    const double v = cell_values_[2][index] * ri;
    const double w = cell_values_[3][index] * ri;
    const double k = 0.5 * (u * u + v * v + w * w);
    const double e = cell_values_[4][index] * ri - k;
    ...
    for (int icomp = 0; icomp < 5; icomp++) {
      for (int idir = 0; idir < 3; idir++) {
        gradient[icomp][idir] = primitives[icomp] * face_normal_[idir][i];
      }
    }
    ...
    for (int icomp = 0; icomp < 5; ++icomp) {
      for (int idir = 0; idir < 3; ++idir) {
        ...
        Kokkos::atomic_fetch_add(cell,
                                 gradient[icomp][idir]
        cell_volumes_(index));*/
        cells.cell_gradient_[icomp][idir][index][0] +=
            gradient[icomp][idir] / cell_volumes_[index];
      }
    }
#endif
    ...
    for (int icomp = 0; icomp < 5; ++icomp) {
      for (int idir = 0; idir < 3; ++idir) {
        cells.cell_gradient_[index][cell_flux_index_[i][0]][icomp][idir] =
            gradient[icomp][idir] / cell_volumes_[index];
      }
    }
#endif
...
struct green_gauss_gradient_sum {
  ...
  void operator()(int i) const {
...
#endif
    {
      ...
      for (int icomp = 0; icomp < 5; icomp++) {
        for (int idir = 0; idir < 3; idir++) {
          gradient_[icomp][idir][i] += face_gradient_[icomp][idir][i][iface];
        }
      }
    }
  }
...
/*GreenGauss
 * contains all of the functions need to compute and communicate cell
 * gradients. aggregates the functors that are needed to compute and sum
 * boundary and internal contributions also communicates the cell
 * gradient data on ghosted cells.
 */
class GreenGauss {
  typedef typename ViewTypes::scalar_field_type scalar_field_type;
  typedef typename ViewTypes::solution_field_type solution_field_type;
 ...
 public:
  ...
  GreenGauss(Faces *internal_faces, std::vector<Faces *> *bc_faces,
  ...
  // ghosted_gradient_vars("ghosted_gradient_vars",
  // total_recv_count*5*3),
  ...
  void compute_gradients(solution_field_type sol_np1_vec,
                         gradient_field_type gradients) {
    // Internal Faces
    const int ninternal_faces = internal_faces_->nfaces_;
    green_gauss_face face_gradient(*internal_faces_, *cells_);
    for (int i = 0; i < ninternal_faces; i++)
      face_gradient(i, *cells_, sol_np1_vec);
    ...
    for (; bcf_iter != bcf_iter_end; ++bcf_iter) {
      ...
      green_gauss_boundary_face bc_gradient(*faces, *cells_);
      for (int i = 0; i < nboundary_faces; i++)
        bc_gradient(i, *cells_, sol_np1_vec);
      // Kokkos::parallel_for(nboundary_faces, bc_gradient);
    }

    // Sum of all contributions.
    green_gauss_gradient_sum gradient_sum(*cells_, gradients);
    for (int i = 0; i < mesh_data_->num_owned_cells; i++) gradient_sum(i);
  ...
  /*void communicate_gradients(gradient_field_type gradients){
    ...
    extract_shared_tensor<5, 3> extract_shared_gradients(gradients,
  mesh_data_->send_local_ids, shared_gradient_vars);//sol_np1_vec,
  send_local_ids, shared_cells);
    //
  Kokkos::parallel_for(mesh_data_->num_ghosts,extract_shared_gradients);
    // Kokkos::fence();
    // Kokkos::deep_copy(shared_gradient_vars_host,
  shared_gradient_vars);

    communicate_ghosted_cell_data(mesh_data_->sendCount,
  mesh_data_->recvCount,
  shared_gradient_vars_host.ptr_on_device(),ghosted_gradient_vars_host.ptr_on_device(),
  15);

    //copy values to be sent from host to device
    // Kokkos::deep_copy(ghosted_gradient_vars,
  ghosted_gradient_vars_host);
    // insert_ghost_tensor<5, 3> insert_ghost_gradients(gradients,
  mesh_data_->recv_local_ids, ghosted_gradient_vars);
    // Kokkos::parallel_for(mesh_data_->num_ghosts,
  insert_ghost_gradients);
    // Kokkos::fence();
  }*/
