 ...
 * See Roe, Journal of Computational Physics, Volume 43, Issue 2, October 1981,
 * Pages 357–372.
 */
struct roe_flux {
  ...
  void compute_flux(const double* const& primitives_left,
                    const double* const& primitives_right, double* const& flux,
    ...
    const double total_enthalpy_left =
        enthalpy_left + 0.5 * (uvel_left * uvel_left + vvel_left * vvel_left +
                               wvel_left * wvel_left);
    const double mass_flux_left =
        rho_left * (face_normal[0] * uvel_left + face_normal[1] * vvel_left +
    ...
    const double mass_flux_right =
        rho_right * (face_normal[0] * uvel_right + face_normal[1] * vvel_right +
    ...
    // const double face_normal_norm = MathTools::Vec3Norm(face_normal);
    const double face_tangent_norm = std::sqrt(
        face_tangent[0] * face_tangent[0] + face_tangent[1] * face_tangent[1] +
        face_tangent[2] * face_tangent[2]);

    // const double face_binormal_norm = MathTools::Vec3Norm(face_binormal);
    const double face_binormal_norm =
        std::sqrt(face_binormal[0] * face_binormal[0] +
                  face_binormal[1] * face_binormal[1] +
                  face_binormal[2] * face_binormal[2]);

    const double face_normal_unit[] = {face_normal[0] / face_normal_norm,
                                       face_normal[1] / face_normal_norm,
                                       face_normal[2] / face_normal_norm};
    const double face_tangent_unit[] = {face_tangent[0] / face_tangent_norm,
                                        face_tangent[1] / face_tangent_norm,
                                        face_tangent[2] / face_tangent_norm};
    const double face_binormal_unit[] = {face_binormal[0] / face_binormal_norm,
                                         face_binormal[1] / face_binormal_norm,
    ...
    const double ubar = uvel_roe * face_normal[0] + vvel_roe * face_normal[1] +
    ...
    MathTools::MatVec5(1.0, roe_mat_eigenvectors, conserved_jump, 0.0, ldq);

    for (int j = 0; j < 5; ++j) ldq[j] = (eigp[j] - eigm[j]) * ldq[j];
    ...
    roe_mat_eigenvectors[5] =
        (uvel_roe + face_normal_unit[0] * speed_sound_roe) *
    ...
    roe_mat_eigenvectors[20] = (enthalpy_roe + kinetic_energy_roe +
                                speed_sound_roe * normal_velocity) *
                               half_speed_sound_squared_inverse;
    roe_mat_eigenvectors[21] = (enthalpy_roe + kinetic_energy_roe -
                                speed_sound_roe * normal_velocity) *
                               half_speed_sound_squared_inverse;
    ...
    MathTools::MatVec5(1.0, roe_mat_eigenvectors, ldq, 0.0, rlldq);

    for (int icomp = 0; icomp < 5; ++icomp) flux[icomp] -= 0.5 * rlldq[icomp];
  }
};
