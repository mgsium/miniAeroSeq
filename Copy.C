...
void copy_faces(Faces device_faces, std::vector<Face>& mesh_faces) {
  ...
  /*if(device_faces.face_cell_conn_.extent(0) > 0) {
    ...
    view_type face_cell_left =
  Kokkos::subview(device_faces.face_cell_conn_,Kokkos::ALL(),0);

    typedef Kokkos::MinMax<int,Device> reducer_type;
    typedef typename reducer_type::value_type minmax_type;
    minmax_type minmax;
    Kokkos::parallel_reduce(face_cell_left.extent(0), KOKKOS_LAMBDA (const int&
  i, minmax_type& lminmax) { if(face_cell_left(i)<lminmax.min_val)
  lminmax.min_val = face_cell_left(i); if(face_cell_left(i)>lminmax.max_val)
  lminmax.max_val = face_cell_left(i);
    },reducer_type(minmax));

    Kokkos::BinSort<view_type, CompType, Device, int>
  bin_sort(face_cell_left,CompType(face_cell_left.extent(0)/2,minmax.min_val,minmax.max_val),true);
